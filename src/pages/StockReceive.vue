...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_재고관리_원자재입고등록.xlsx"; // 엑셀 양식주소
  ...
  if (event) {
    ...
    reader.onload = async (e) => {
      ...
      file_data.value.forEach((fd: any) => {
        if (isNaN(Date.parse(fd.입고일시.toLocaleString())))
          fd.입고일시 = moment().format("YYYY-MM-DD HH:mm:ss");
